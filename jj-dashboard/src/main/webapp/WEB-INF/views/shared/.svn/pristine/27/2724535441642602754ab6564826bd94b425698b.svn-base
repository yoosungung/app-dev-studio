        ...
        <div class="table full" v-if="model.tbSysAuthor.userAuthorYn">
            ...
            <div style="width: 33%;">
                <div class="table full">
                    <h3>권한별 보직</h3>
                    <div class="tar">
                        <ui-list-button-append v-bind:data="model.tbSysAuthorRelateCodeDtyNmList"
                            v-bind:new-row-data="{ relateCodeAuthorYn: true }"
                            id-column-name="dtyId"
                        ></ui-list-button-append>
                        <ui-list-button-delete v-bind:data="model.tbSysAuthorRelateCodeDtyNmList"></ui-list-button-delete>
                    </div>
                </div>
                <table class="list auto">
                    <thead>
                        <tr>
                            <th width="40px">
                                <ui-checkbox-head v-bind:data="model.tbSysAuthorRelateCodeDtyNmList"></ui-checkbox-head>
                            </th>
                            <th width="">보직명</th>
                        </tr>
                    </thead>
                    <tbody v-cloak v-if="tbSysAuthorRelateCodeDtyNmFiltered.length == 0">
                        <tr>
                            <td colspan="2" class="tac">(보직 없음)</td>
                        </tr>
                    </tbody>
                    <tbody v-cloak v-if="tbSysAuthorRelateCodeDtyNmFiltered.length > 0">
                        <tr v-for="row in tbSysAuthorRelateCodeDtyNmFiltered" v-bind:key="row.userId">
                            <td class="tac">
                                <ui-checkbox-row v-bind:data="row"></ui-checkbox-row>
                            </td>
                            <td class="tal">
                                <ui-input type="text" v-model="row.relateCodeValue" v-bind:max-byte="50" v-bind:byte-indicator="false"></ui-input>
                                <ui-valid-checker v-bind:value="row.relateCodeValue" check="required"></ui-valid-checker>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
vues.edit = new Vue({
    ...
    computed: {
        ...
        personSeList: function() {
            return this.model.tbSysAuthorRelateCodePersonSeList;
        },
        tbSysAuthorRelateCodeDtyNmFiltered: function() {
            return CommonUtil.getFilteredList(this.model.tbSysAuthorRelateCodeDtyNmList);
        }
    },
    mounted: function() {
        var vue = this;
