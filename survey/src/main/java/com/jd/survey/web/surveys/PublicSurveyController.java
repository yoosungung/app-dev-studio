...
import com.jd.survey.dao.survey.QuestionStatisticMSSQLDAOImp;
		...
		try{
			...
			//if (surveyDefinition.getAllowMultipleSubmissions()) {//allow multiple submissions of this survey from the same client IP Address
				//if (userSurveyEntries == null || userSurveyEntries.size() == 0) {	//No User entries for this survey, create a new one
					...
					return "redirect:/surveys/open/" + encodeUrlPathSegment(survey.getId().toString(), httpServletRequest) +"/1";
				/*} 
				else {//entries found 
					...
					uiModel.addAttribute("survey_base_path", "surveys/open");
			...
			/*else {
				...
				if (userSurveyEntries == null || userSurveyEntries.size() == 0) {
					...
					System.out.println("surveyAlreadyTaken");
		...
		try {
			if(proceedAction!=null){ //submit button
				...
				if (surveyDefinition.getAllowMultipleSubmissions()) {
					return "redirect:/surveys/open/" + encodeUrlPathSegment(survey.getTypeId().toString(), httpServletRequest) + "?list";	
				}
				else
				{
					uiModel.asMap().clear();
			...
			else
			{
				...
				return "redirect:/surveys/open/" + encodeUrlPathSegment(survey.getId().toString(), httpServletRequest) +"/" + encodeUrlPathSegment(order.toString(), httpServletRequest);
			}
	...
	public String showSurvey(@PathVariable("id") Long surveyId,
		...
		try{
			...
			if (survey.getStatus() == SurveyStatus.I) {
				return "redirect:/surveys/open/" + encodeUrlPathSegment(surveyId.toString(), httpServletRequest) +"/1";
			}
			else
			{
				uiModel.addAttribute("survey_base_path", "surveys/open");
	...
	try {
			...
			if(proceedAction != null){ //next button
				...
				if (order.equals((short)0)){
					//Submit page
					uiModel.asMap().clear();
					return "redirect:/surveys/open/submit/" + encodeUrlPathSegment(surveyPage.getSurvey().getId().toString(), httpServletRequest);	
				}
				else{
					//go to the next page
					uiModel.asMap().clear();
					return "redirect:/surveys/open/" + encodeUrlPathSegment(surveyPage.getSurvey().getId().toString(), httpServletRequest) +"/" + encodeUrlPathSegment(order.toString(), httpServletRequest);	
				}
