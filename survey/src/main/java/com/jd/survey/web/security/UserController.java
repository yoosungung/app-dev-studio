...
//				}
				...
				return "redirect:/security/users/saved";
			}
			else {
	...
	public String list_external(
		...
		if(StringUtils.hasText(keywords)) {
			uiModel.addAttribute("users", userService.user_findAllExternal(firstResult, sizeNo, keywords));
			total = (float) userService.user_getCountExternal(keywords);
			nrOfPages = total / sizeNo;
		}
		else {
			uiModel.addAttribute("users", userService.user_findAllExternal(firstResult, sizeNo));
			total = (float) userService.user_getCountExternal();
			nrOfPages = total / sizeNo;
		}
		uiModel.addAttribute("total", (int) total);
		uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
	...
	public String search_byName(Principal principal, Model uiModel) {
		try {
	...
	public String search_byLogin(Principal principal, Model uiModel) {
		try {
			uiModel.addAttribute("newSearch",true);
	...
	public String search_byEmail(Principal principal, Model uiModel) {
		try {
			uiModel.addAttribute("newSearch",true);
	...
	public String updateUserPrepare(@PathVariable("id") Long id, Principal principal, Model uiModel) {
		...
		try{
			...
			if(proceed != null){
				...
				if (userService.user_findByLogin(user.getLogin()) != null && userService.user_ValidateLoginIsUnique(user) == true) {
					bindingResult.rejectValue("login", "field_unique");
					populateEditForm(uiModel, user,loggedInUser);
					redirectAttr.addFlashAttribute("login_error", "입력한 값이 이미 사용중입니다.");
					return "redirect:/security/users/"+user.getId()+"?form=";
				}
				if (userService.user_findByEmail(user.getEmail()) != null && userService.user_ValidateEmailIsUnique(user) == true){
					bindingResult.rejectValue("email", "field_unique");
					populateEditForm(uiModel, user,loggedInUser);
					redirectAttr.addFlashAttribute("email_error", "입력한 값이 이미 사용중입니다.");
					return "redirect:/security/users/"+user.getId()+"?form=";
				}
				uiModel.asMap().clear();
				user = userService.user_merge(user);
	...
	public String updatePasswordPrepare(@PathVariable("id") Long id, Principal principal, Model uiModel) {
		log.info("updateForm(): id=" + id);
		try{
			User loggedInUser = userService.user_findByLogin(principal.getName());
			populateEditForm(uiModel, userService.user_findById(id),loggedInUser);
...
//				}
	...
	void populateEditForm(Model uiModel, User user, User loggedInUser ) {
		try {
			...
			uiModel.addAttribute("surveyDefinitions", surveySettingsService.surveyDefinition_findAllForUserListInternal(loggedInUser));
