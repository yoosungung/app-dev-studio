...
public class SurveyDefinitionController {
	...
	public String show(@PathVariable("id") Long id, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
	 ...
	 */
	@Secured({"ROLE_ADMIN","ROLE_SURVEY_ADMIN"})
	@RequestMapping(value = "/{id}/old", produces = "text/html")
	public String show_(@PathVariable("id") Long id, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		...
		try {
			...
			uiModel.addAttribute("isShow", true);
			uiModel.addAttribute("itemId", id);
			return "settings/surveyDefinitions/show_";
		} catch (Exception e) {
			log.error(e.getMessage(),e);
			throw (new RuntimeException(e));
		}
	 ...
	 */
	@Secured({"ROLE_ADMIN","ROLE_SURVEY_ADMIN"})
	@RequestMapping(value = "/upload/old", produces = "text/html")
	public String uploadFromJsonOld(@RequestParam(value = "id", required = false) Long departmentId,
			Model uiModel, Principal principal, HttpServletRequest httpServletRequest) {
		...
		try {
			...
			return "settings/surveyDefinitions/upload_";
		} 
		catch (Exception e) {
			log.error(e.getMessage(),e);
			throw (new RuntimeException(e));
	 ...
	 */
	@Secured({"ROLE_ADMIN","ROLE_SURVEY_ADMIN"})
	@RequestMapping(value = "/{id}/old", params = "form", produces = "text/html")
	public String updateFormOld(@PathVariable("id") Long id, Principal principal, Model uiModel) {
		log.info("updateForm(): id=" + id);
		try{
			User user = userService.user_findByLogin(principal.getName());
			populateEditForm(uiModel, surveySettingsService.surveyDefinition_findById(id), user);
			return "settings/surveyDefinitions/update_";
		} catch (Exception e) {
			log.error(e.getMessage(),e);
			throw (new RuntimeException(e));
		}
	...
	public String update(@RequestParam(value = "_proceed", required = false) String proceed,
			@Valid SurveyDefinition surveyDefinition, BindingResult bindingResult,
			Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
	...
	public void getSurveyQRCode(@PathVariable("id") Long surveyDefinitionId,
			Model uiModel, Principal principal, HttpServletRequest httpServletRequest, HttpServletResponse response) {
		try {
			uiModel.asMap().clear();
			User user = userService.user_findByLogin(principal.getName());
			//Check if the user is authorized
	...
	public String saveTemplate(@PathVariable("id") Long surveyDefinitionId,
			Principal principal, Model uiModel, HttpServletRequest httpServletRequest, HttpServletResponse response) {
	...
	public void exportToJson(@PathVariable("id") Long surveyDefinitionId,
			Principal principal, HttpServletRequest httpServletRequest, HttpServletResponse response) {
		try {
			String login = principal.getName();
			User user = userService.user_findByLogin(login);
			//Check if the user is authorized
	...
	public String publishSurveyDefinition(@PathVariable("id") Long surveyDefinitionId,
			HttpServletResponse response, Principal principal, Model uiModel, HttpServletRequest httpServletRequest){
		try{ 
			String login = principal.getName();
			User user = userService.user_findByLogin(login);
			//Check if the user is authorized
