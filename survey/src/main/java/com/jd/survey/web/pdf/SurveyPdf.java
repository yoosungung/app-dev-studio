...
public class SurveyPdf  extends AbstractPdfView{
	...
	private static final Font headTitleFont = new Font(getBaseFont(), 13, Font.BOLD, new Color(0,68,136));
	
	private static final Font titleFont = new Font(getBaseFont(), 11, Font.BOLD, new Color(0,68,136));
	
	private static final Font boldedFont = new Font(getBaseFont(), 9, Font.NORMAL);
	
	private static final Font normalFont = new Font(getBaseFont(), 9, Font.NORMAL);
	
	//~ Methods ============================================================================
	
	public static BaseFont getBaseFont() {
		String font = "/itext/HYGPRM.TTF";
	...
	protected void buildPdfDocument(Map model, Document document,
		...
		String trueMessage = "예";
		String dateFormat = (String)  model.get("dateFormat");
		
		
		
		PdfContentByte canvas = writer.getDirectContentUnder();
        Image image = Image.getInstance(this.getClass().getResource("/itext/background.jpg"));
        image.scaleAbsolute(595, 840);
		...
		writeHeadTitle(document, surveyEntry.getSurveyName());
		writeTitle(document, "설문조사 기본정보");
		writeAnswer(document, "아이디", surveyEntry.getSurveyId().toString());	
		writeAnswer(document, "작성자", surveyEntry.getCreatedByFirstName());
		writeAnswer(document, "작성일", surveyEntry.getCreationDate() == null ? "" : DateValidator.getInstance().format(surveyEntry.getCreationDate(), dateFormat));
		writeAnswer(document, "제출일", surveyEntry.getSubmissionDate() == null ? "" : DateValidator.getInstance().format(surveyEntry.getSubmissionDate(), dateFormat));
		...
		for(SurveyPage surveyPage :surveyPages) {
			...
			for(QuestionAnswer questionAnswer :surveyPage.getQuestionAnswers()) {
				...
				questionAnswer.getQuestion().setQuestionText(questionAnswer.getQuestion().getOrder() + ". " + cr.getCleanHTML().replaceAll("\n", " "));

				switch (questionAnswer.getQuestion().getType()) {
				case YES_NO_DROPDOWN:
					writeAnswer(document,questionAnswer.getQuestion().getQuestionText(), questionAnswer.getBooleanAnswerValue(), falseMessage,trueMessage);							
					break;
				...
				case VIDEO_DISPLAY:
					break;
				case FILE_UPLOAD:
					writeAnswer(document,questionAnswer.getQuestion().getQuestionText(), questionAnswer.getSurveyDocument() != null ?questionAnswer.getSurveyDocument().getFileName(): "");
					break;
				case STAR_RATING:
					//verify that the value exists in the options
					for (QuestionOption option: questionAnswer.getQuestion().getOptions()){
	...
	private void writeHeadTitle(Document document, String title) throws Exception{
		Paragraph titleParagraph = new Paragraph(title,headTitleFont);
	...
	private void writeAnswer(Document document, String questionText, boolean answerValue, String falseMessage, String trueMessage) throws Exception{
