...
public class Group implements  Comparable <Group> ,Serializable, SecurityObject {
	...
	@JoinTable(name="survey_sec_group_authority",joinColumns={@JoinColumn(name="group_id", referencedColumnName="id")},inverseJoinColumns={@JoinColumn(name="authority_id", referencedColumnName="id")})
	private SortedSet<Authority> authorities  = new TreeSet<Authority>();
	
	@NotNull
	@ManyToMany
	@Sort(type = SortType.NATURAL)
	@JoinTable(name="survey_sec_user_group",joinColumns={@JoinColumn(name="group_id", referencedColumnName="id")},
