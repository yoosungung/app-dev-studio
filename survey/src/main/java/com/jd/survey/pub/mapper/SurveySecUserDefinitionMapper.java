...
public interface SurveySecUserDefinitionMapper<T extends Serializable> {
	 ...
	 * 사용자설문조사 건수
	 */
	int getUserSurveyDefinitionCount(@Param("surveyDefinitionId") Long surveyDefinitionId, @Param("userId") Long userId);

	/**
	 * 사용자설문조사 목록 조회
	 */
	List<SurveySecUserDefinition> getUserSurveyDefinitionsBySurveyDefinitionId(@Param("surveyDefinitionId") long surveyDefinitionId);
