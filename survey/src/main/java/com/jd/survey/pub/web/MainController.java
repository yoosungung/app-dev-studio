...
public class MainController {
	...
	@Autowired private ZeusQuestionAndAnswerService zeusQuestionAndAnswerService;
	...
	public String main(SurveyDefinitionCmd cmd, Principal principal, ModelMap model) {
		...
		//로그인에 따른 실태조사 상태별 목록
		if(principal != null) {
			model.addAttribute("actualSurveyList", surveyDefinitionService.getMainRealSurveyDefinitionListByLogin(principal.getName()));
		}
		else {
		...
		//qna 
		//model.addAttribute("qnaList", zeusQuestionAndAnswerService.getQuestionAndAnswerList());
