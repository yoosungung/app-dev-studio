...
public class SurveyDefinitionController {
	...
	@Autowired	private PubSurveyService pubSurveyService;
	...
	public String process(@PathVariable long id, Principal principal, ModelMap model, HttpServletRequest httpServletRequest) {
		...
		if(surveyDefinitionService.getSurveyDefinition(id).getStatus().equals("E") ) {
			...
			for(SurveyDefinition processSurvey : processSurveyList) {
				...
				if(id == processSurvey.getSurveyDefinitionId()) {
					...
					if(principal != null && pubSurveyService.getSurveyCount(processSurvey.getSurveyDefinitionId(), principal.getName())>0) {
							List<SurveyPage> surveyPages = surveyService.surveyPage_getAll(pubSurveyService.getSurvey(processSurvey.getSurveyDefinitionId(), principal.getName()).getSurveyId(), messageSource.getMessage(DATE_FORMAT, null, LocaleContextHolder.getLocale()));
							
							processSurvey.setQuestionAnswers(surveyPages.get(0).getQuestionAnswers());
							processSurvey.setAnswerStatus(pubSurveyService.getSurvey(processSurvey.getSurveyDefinitionId(), principal.getName()).getStatus());
							processSurvey.setSurveyId(pubSurveyService.getSurvey(processSurvey.getSurveyDefinitionId(), principal.getName()).getSurveyId());
					}
					else {
						for(SurveyDefinitionPage surveyDefinitionPage : surveySettingsService.surveyDefinition_findById(processSurvey.getSurveyDefinitionId()).getPages()) {
	...
	public String vote(@Valid SurveyPage surveyPage, Principal principal, ModelMap model, HttpServletRequest httpServletRequest) {
		
		Survey survey = surveyService.survey_create(surveyPage.getSurvey().getTypeId(),principal.getName(), httpServletRequest.getRemoteAddr());
	...
	public String done(@PathVariable long id, Principal principal, ModelMap model, HttpServletRequest httpServletRequest) {
		
		if(principal != null && pubSurveyService.getSurveyCount(id, principal.getName())>0) {
			List<SurveyPage> surveyPages = surveyService.surveyPage_getAll(pubSurveyService.getSurvey(id, principal.getName()).getSurveyId(), messageSource.getMessage(DATE_FORMAT, null, LocaleContextHolder.getLocale()));
	...
	public String targetSearch(@PathVariable long id, SurveySecUserDefinitionCmd cmd, ModelMap model) {
		...
		for(SurveyDefinition  surveyDefinition: surveyDefinitionService.getRealSurveyDefinitionList()) {
			if(id == surveyDefinition.getDepartmentId()) cmd.setSurveyDefinitionId(surveyDefinition.getSurveyDefinitionId());
		}
	...
	public @ResponseBody List<SurveyDefinitionStatus> surveyDefinitionStatus(@PathVariable long id, @PathVariable int order) {
		if(order == 1) return pubSurveyService.getOrganStatus(id);
		else return pubSurveyService.getAreaStatus(id);
	}
	...
	public long getNextId(List<SurveyDefinition> list,  SurveyDefinition surveyDefinition) {
	    ...
	    if (idx < 0 || idx+1 == list.size()) return 0;
