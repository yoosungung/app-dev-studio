...
public class DataSetController {
   ...
   //renders the page for the survey definition import from json file
	...
	public String prepareForImport(@PathVariable("id") Long dataSetId, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		try {
	...
	public String createPost(@RequestParam(value = "_proceed", required = false) String proceed,
			@Valid DataSet dataSet, BindingResult bindingResult,
			Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		...
		try {
			...
			if(proceed != null) {
				...
				if (surveySettingsService.dataset_findByName(dataSet.getName()) != null && !surveySettingsService.dataset_findByName(dataSet.getName()).getId().equals(dataSet.getId())){
