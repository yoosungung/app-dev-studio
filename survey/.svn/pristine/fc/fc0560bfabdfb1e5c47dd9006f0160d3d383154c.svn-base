...
public class QuestionController {
    ...
    @Secured({"ROLE_ADMIN","ROLE_SURVEY_ADMIN"})
    @RequestMapping(value = "/{id}/old", params = "create", produces = "text/html")
    public String createQuestionOld(@PathVariable("id") Long surveyDefinitionPageId,
            Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
        log.info("createForm(): handles param form");
        try {
            String login = principal.getName();
            ...
            return "settings/questions/create_";
        } catch (Exception e) {
            log.error(e.getMessage(),e);
            throw (new RuntimeException(e));
        }
    ...
    public String updateFormOld(@PathVariable("id") Long id, Principal principal, Model uiModel) {
        ...
        try{
            User user = userService.user_findByLogin(principal.getName());
            populateEditForm(uiModel, surveySettingsService.question_findById(id),user);
            return "settings/questions/update_";
        } catch (Exception e) {
            log.error(e.getMessage(),e);
            throw (new RuntimeException(e));
        }
    ...
    public String show(@PathVariable("id") Long id, HttpServletRequest httpServletRequest, Principal principal, Model uiModel) {
        log.info("show(): id=" + id);
        try {
            String login = principal.getName();
            User user = userService.user_findByLogin(login);
