...
public class PrivateSurveyController {
	...
	public String listSurveyEntries(@PathVariable("id") Long surveyDefinitionId, 
		...
		try{
			...
			if (userSurveyEntries == null || userSurveyEntries.size() == 0) {
				...
				return "redirect:/surveys/private/" + encodeUrlPathSegment(survey.getId().toString(), httpServletRequest) +"/1";
			} 
			//entries found
			else { 
	...
	public String modSurvey(@PathVariable("id") Long surveyId, 
							 		Principal principal,
							 		HttpServletRequest httpServletRequest) {
		try{
			String login = principal.getName();
			...
			if(surveyService.modSurveyStatus(surveyId, login) > 0) {
				return "redirect:/surveys/private/" + encodeUrlPathSegment(surveyId.toString(), httpServletRequest) +"/1";	
			}
			else {
				return "redirect:/surveys/private/" + encodeUrlPathSegment(surveyId.toString(), httpServletRequest) +"?list";
			}
...
//				return "redirect:/surveys/private/" + encodeUrlPathSegment(survey.getTypeId().toString(), httpServletRequest) + "?list";
			...
			else
			{
				...
				return "redirect:/surveys/private/" + encodeUrlPathSegment(survey.getId().toString(), httpServletRequest) +"/" + encodeUrlPathSegment(order.toString(), httpServletRequest);
			}
	...
	public String showSurvey(@PathVariable("id") Long surveyId,
		...
		try{
			...
			else
			{
				...
				String status = null;
				for(com.jd.survey.pub.domain.SurveyDefinition surveyDefinition : surveyDefinitionService.getRealSurveyDefinitionList()) {
		...
		String login = principal.getName();
			...
			if(proceedAction!=null){ //next button
				...
				if (order.equals((short)0)){
					//Submit page
					uiModel.asMap().clear();
					return "redirect:/surveys/private/submit/" + encodeUrlPathSegment(surveyPage.getSurvey().getId().toString(), httpServletRequest);	
				}
				else{
					//go to the next page
					uiModel.asMap().clear();
					return "redirect:/surveys/private/" + encodeUrlPathSegment(surveyPage.getSurvey().getId().toString(), httpServletRequest) +"/" + encodeUrlPathSegment(order.toString(), httpServletRequest);	
				}
