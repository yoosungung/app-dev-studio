...
public class UserDAOImpl extends AbstractJpaDao<User> implements UserDAO {
	...
	public Set<User> findAllExternal(int startResult, int maxRows, String keywords)throws DataAccessException {
		Query query = createNamedQuery("User.findAllExternalAndSearch", startResult, maxRows, "%" + keywords + "%");
		return new LinkedHashSet<User>(query.getResultList());
	}
	...
	public Long getCountExternal(String keywords) throws DataAccessException {
		try {
			Query query = createNamedQuery("User.getCountExternalAndSearch",-1,-1, "%" + keywords + "%");
			return  (Long) query.getSingleResult();
		} catch (NoResultException nre) {
