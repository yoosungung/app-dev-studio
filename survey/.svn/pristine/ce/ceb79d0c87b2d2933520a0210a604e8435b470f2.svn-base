...
public class SurveyDefinitionPageController {
	...
	public String updateFormOld(@PathVariable("id") Long id, Principal principal, Model uiModel) {
		try {
			User user = userService.user_findByLogin(principal.getName());	
			populateEditForm(uiModel, surveySettingsService.surveyDefinitionPage_findById(id),user);
			return "settings/surveyDefinitionPages/update_";
		} catch (Exception e) {
			log.error(e.getMessage(),e);
			throw (new RuntimeException(e));
		}
	...
	public String update(@RequestParam(value = "_proceed", required = false) String proceed,
			@Valid SurveyDefinitionPage surveyDefinitionPage,BindingResult bindingResult, 
	...
	@Secured({"ROLE_ADMIN","ROLE_SURVEY_ADMIN"})
	@RequestMapping(value = "/{id}/old", params = "fork", produces = "text/html")
	public String prepareForkFormOld(@PathVariable("id") Long id, Principal principal, Model uiModel) {
		try{
			SurveyDefinitionPage surveyDefinitionPage =surveySettingsService.surveyDefinitionPage_findById(id);
			surveyDefinitionPage.loadFromJson();
			...
			return "settings/surveyDefinitionPages/fork_";
		} catch (Exception e) {
			log.error(e.getMessage(),e);
			throw (new RuntimeException(e));
		}
	...
	public String show(@PathVariable("id") Long id, Principal principal, HttpServletRequest httpServletRequest, Model uiModel) {
	...
	public String createGet(@PathVariable("id") Long surveyDefinitionId, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		try {
			User user = userService.user_findByLogin(principal.getName());
