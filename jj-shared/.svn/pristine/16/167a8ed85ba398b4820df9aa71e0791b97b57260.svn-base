...
import kr.ac.jj.shared.infrastructure.framework.mybatis.persistence.dao.helper.SqlMapHelper;
...
public class SelectQueryWrappingInterceptor implements Interceptor {
    ...
    private void processIntercept(Invocation invocation) {
        ...
        SqlMapHelper sqlMapHelper = SqlMapHelperUtil.getSqlMapHelper(dataSource);

        for (Entry<String, Object> entry : parameter.entrySet()) {
            if (StringUtils.equals(entry.getKey(), "paging") && entry.getValue() instanceof PagingInfo) {
                sqlMapHelper.setPagingInfo((PagingInfo) entry.getValue());
                break;
            }
        }

        parameter.put(SqlMapHelper.WRAPPER_KEY, sqlMapHelper);
    }
