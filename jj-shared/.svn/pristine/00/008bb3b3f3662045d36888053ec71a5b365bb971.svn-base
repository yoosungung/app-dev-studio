...
Vue.component('ui-date', {
    ...
    template: ' \
        <div class="dateBox"> \
            <input type="text" v-model="modelValue" readonly> \
            <a href="javascript:void(0);" class="hide_txt iconBtn ico_date">달력</a> \
        </div> \
    ',
    computed: {
        modelValue: {
    ...
    mounted: function() {
        ...
        $elm.find(".ico_date").click(function(){
...
Vue.component('ui-grid-paging', {
    ...
    computed: {
        titleMessage: function() {
            ...
            if (this.result.error === true) {
                ...
                titleMessage = '총 <b>' + this.result.paging.totalRecordCount + '</b> 건';
    ...
    methods: {
        initSortColumns: function() {
            ...
            var $ths = $table.children('thead').find('th[data-sort-name]');
            var orderByColumns = vue.request.paging.orderBy.columns;

            $ths.each(function() {
                var $th = $(this);

                if ($th.data("sort-name") == "" || $th.children('span.sort').length > 0) {
                    return;
                }

                var $span = $('<span class="sort"></span>').prependTo(this);

                if (orderByColumns != null && orderByColumns[$th.data("sort-name")] != null) {
                    $span.addClass(orderByColumns[$th.data("sort-name")].toLowerCase());
                }
                ...
                $th.css({ "cursor": "pointer" }).click(function() {
                    ...
                    vue.request.paging.orderBy.columns[$th.data("sort-name")] = orderByDirection;
...
Vue.component('ui-list-button-append', {
    ...
    template: ' \
        <a href="javascript:void(0);" class="tableBtn" v-on:click="appendRow();"> \
            <slot>추가</slot> \
        </a> \
    ',
    methods: {
...
Vue.component('ui-list-button-delete', {
    ...
    template: ' \
        <a href="javascript:void(0);" class="tableBtn" v-on:click="deleteSelectedRow();"> \
            <slot>삭제</slot> \
        </a> \
    ',
    methods: {
