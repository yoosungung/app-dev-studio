...
public class SqlVariables {
    ...
    protected String getCodeData(String codeDataPath, String columnName, String defaultText, Locale locale) {
        ...
        boolean booleanType = this.isBooleanType(codeDataList);

        sb.append("CASE ").append(columnName);

        for (int i = 0; i < codeDataList.size(); i++) {
            String code = (String) codeDataList.get(i, "code");
    ...
    private boolean isBooleanType(BaseMapList codeDataList) {
        for (int i = 0; i < codeDataList.size(); i++) {
            if (!StringUtils.equalsAnyIgnoreCase((String) codeDataList.get(i, "code"), "true", "false")) {
                return false;
            }
        }
