...
import kr.ac.jj.survey.infrastructure.framework.web.servlet.progress.ServletProgressContainer;
...
public class GridDataResultHandler extends DataResultHandler {
    ...
    private void initGridDataWriter() {
        if (this.gridRequest.getExcel() == null || BooleanUtils.isFalse(this.gridRequest.getExcel().getDownload())) {
            this.gridDataWriter = new GridDataWriterToJson(this.gridRequest);
        } else {
            this.gridDataWriter = new GridDataWriterToExcelWithPOI(this.gridRequest);
        }
    }
