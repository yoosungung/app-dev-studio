...
public class ZeusUserController {
    ...
    public String change(@PathVariable("id") String id) {
        ...
        if (user != null) {
            context.setAuthentication(
                    new UsernamePasswordAuthenticationToken(user, passwordEncoder.encode("1"), user.getAuthorities()));
        }
    ...
    public @ResponseBody int delUser(String surveyId, String userId) {
        ...
        if (userId.equals("all")) {
            ...
            userService.delUserSurveyDefinitionBySurveyDefinitionIdAndUserId(Long.valueOf(surveyId).longValue(),
                    user.getId());
        }
        return result;
    }
