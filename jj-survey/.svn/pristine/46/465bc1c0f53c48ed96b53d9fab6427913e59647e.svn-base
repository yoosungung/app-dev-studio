...
public class DataSetController {
    ...
    public void getExampleCsvFile(Principal principal, HttpServletResponse response) {
        try {
            ...
            servletOutputStream.write(stringBuilder.toString().getBytes("EUC-KR"));
    ...
    public void export(@PathVariable("id") Long dataSetId, Principal principal, HttpServletResponse response) {
        try {
            ...
            ServletOutputStream servletOutputStream = response.getOutputStream();
            servletOutputStream.write(commaDelimtedString.getBytes("EUC-KR"));
            servletOutputStream.flush();
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            throw (new RuntimeException(e));
    ...
    public String importDatasetItems(@RequestParam("file") MultipartFile file, @RequestParam("id") Long dataSetId,
        ...
        try {
            ...
            if (proceed != null) {
                ...
                // if file is empty OR the file type is incorrect the upload page is returned
                ...
                try {
                    CSVReader csvReader;
                    csvReader = new CSVReader(new InputStreamReader(file.getInputStream(), "EUC-KR"));
                    surveySettingsService.importDatasetItems(csvReader, dataSetId, ignoreFirstRow);
                    // done Redirect to the set view page
                    return "redirect:/settings/datasets/saved";
                } catch (Exception e) {
                    log.error(e.getMessage(), e);
                    uiModel.addAttribute("dataSet", surveySettingsService.dataSet_findById(dataSetId));
                    uiModel.addAttribute("emptyFileError", true);
    ...
    public String saved() {
        return "settings/datasets/saved";
    }
    ...
    public String createPost(@RequestParam(value = "_proceed", required = false) String proceed, @Valid DataSet dataSet,
    ...
    public String show(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page,
        ...
        try {
            ...
            uiModel.addAttribute("dataSetItems",
                    surveySettingsService.datasetItem_findByDataSetId(id, firstResult, sizeNo));
    ...
    public String updateForm(@PathVariable("id") Long id, Principal principal, Model uiModel) {
        ...
        try {
            ...
            if (!user.isAdmin()) {
                ...
                // httpServletRequest.getPathInfo() + " attempted by user login:" +
                // principal.getName() + "from IP:" + httpServletRequest.getLocalAddr());
                return "accessDenied";
            }
    ...
    void populateEditForm(Model uiModel, DataSet dataSet, User user) {
