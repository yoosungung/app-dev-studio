...
public class QuestionDAOImpl extends AbstractJpaDao<Question> implements QuestionDAO {
    ...
    private final static Set<Class<?>> dataTypes = new HashSet<Class<?>>(
            Arrays.asList(new Class<?>[] { Question.class }));
    ...
    public Question findById(Long id) throws DataAccessException {
        try {
            Query query = createNamedQuery("Question.findById", -1, -1);
    ...
    @Override
    public Question findByOrder(Long surveyDefinitionId, Short pageOrder, Short questionOrder)
            throws DataAccessException {
        try {
            Query query = createNamedQuery("Question.findByOrder", -1, -1);
            query.setParameter("surveyDefinitionId", surveyDefinitionId);
            query.setParameter("pageOrder", pageOrder);
    ...
    public int deleteBySurveyDefinitionPageId(Long id) throws DataAccessException {
        Query query = createNamedQuery("Question.deleteBySurveyDefinitionPageId", 0, 0);
        query.setParameter("id", id);
