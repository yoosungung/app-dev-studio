...
public class QuestionController {
    ...
    @Autowired
    private JdSurveyQuestionServiceImpl jdSurveyQuestionService;
    ...
    public String createQuestion(@PathVariable("id") Long surveyDefinitionPageId, Principal principal, Model uiModel,
            HttpServletRequest httpServletRequest) {
        log.info("createForm(): handles param form");
        try {
            String login = principal.getName();
            User user = userService.user_findByLogin(login);
            SurveyDefinitionPage surveyDefinitionPage = surveySettingsService
                    .surveyDefinitionPage_findById(surveyDefinitionPageId);
            ...
            short size = (short) question.getPage().getQuestions().size();
            populateEditForm(uiModel, question, user, size);
    ...
    public String copyForm(@PathVariable("id") Long id, Principal principal, Model uiModel,
            HttpServletRequest httpServletRequest) {
        log.info("copyForm(): handles param form");
        try {
            String login = principal.getName();
            User user = userService.user_findByLogin(login);
            Question question = surveySettingsService.question_findById(id);
            SurveyDefinitionPage surveyDefinitionPage = surveySettingsService
                    .surveyDefinitionPage_findById(question.getPage().getId());
            // Check if the user is authorized
            if (!securityService.userIsAuthorizedToManageSurvey(surveyDefinitionPage.getSurveyDefinition().getId(),
                    user)
                    && !securityService.userBelongsToDepartment(
            ...
            return "settings/questions/copy";
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            throw (new RuntimeException(e));
        }
    ...
    public String create(@RequestParam(value = "_proceed", required = false) String proceed,
            @RequestParam(required = false) Long questionId, @Valid Question question, BindingResult bindingResult,
        ...
        try {
            ...
            if (proceed != null) {
                ...
                if (questionId != null) {
                    jdSurveyQuestionService.createSubItemCopyList(questionId, question);
                }
    ...
    public String show(@PathVariable("id") Long id, HttpServletRequest httpServletRequest, Principal principal,
            Model uiModel) {
        log.info("show(): id=" + id);
        try {
            String login = principal.getName();
    ...
    public String lookup(Principal principal, Question question, Model uiModel, HttpServletRequest httpServletRequest) {
    ...
    public String update(@RequestParam(value = "_proceed", required = false) String proceed, @Valid Question question,
            BindingResult bindingResult, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
        ...
        try {
            ...
            if (proceed != null) {
                ...
                if (question.getSuportsOptions()) {
                    // If user wants to modify and existent question without options to Rating type,
                    ...
                    if ((question.getType().toString() == "SMILEY_FACES_RATING"
                        ...
                        question = surveySettingsService.question_merge(question, options);
                        uiModel.asMap().clear();
                        return "redirect:/settings/questions/saved";
                    } else {
    ...
    void populateEditForm(Model uiModel, Question question, User user, short size) {
