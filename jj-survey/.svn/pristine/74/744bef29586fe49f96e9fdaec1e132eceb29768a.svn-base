...
public abstract class BaseCmd<E> extends BaseBean {
    ...
    /** 정렬필드 목록 */
    private final String[] BASE_SORTS;
    ...
    public List<E> getPageList() {
        if (getTotal() == 0 || getSource().size() <= getPageSize())
            return source;
        else
            return getSource().subList(getFirstElementOnPage() - 1, getLastElementOnPage());
    }
        ...
        for (PropertyDescriptor prop : props) {
            ...
            if (prop.getWriteMethod() != null && prop.getReadMethod() != null) {
                ...
                if (!ObjectUtils.containsElement(EXCLUDE_PARAMS, prop.getName())) {
                    ...
                    Object fieldValue = FieldUtils.getFieldValue(this, prop.getName());
    ...
    private List<String[]> getNotEmpytCommandParameters() {
        ...
        for (String[] param : getParameters()) {
            if (!ObjectUtils.containsElement(EXCLUDE_COMMANDS, param[0]) && StringUtils.hasText(param[1]))
    ...
    public String getParameterUrl() {
        ...
        return ((url == null) ? "?" : url + "&").concat("sort=" + getSort()).concat("&sortOrder=" + getSortOrder())
                .concat("&pageSize=" + getPageSize()).concat("&page=" + getPage());
    }
    ...
    public String getPageParameterUrl() {
        String url = getCommandParameterUrl();
        return ((url == null) ? "?" : url + "&").concat("sort=" + getSort()).concat("&sortOrder=" + getSortOrder())
                .concat("&pageSize=" + getPageSize()).concat("&page=");
    }
    ...
    public String getSortParameterUrl() {
        String url = getCommandParameterUrl();
        return ((url == null) ? "?" : url + "&").concat("page=" + getPage()).concat("&pageSize=" + getPageSize())
                .concat("&sort=");
    }
