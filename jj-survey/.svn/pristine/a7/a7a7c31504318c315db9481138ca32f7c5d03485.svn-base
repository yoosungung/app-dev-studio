...
public class SurveyService {
    ...
    @Autowired
    private SurveyDocumentDAO surveyDocumentDAO;

    @Autowired
    @Qualifier(value = "com.jd.surveyMapper")
    private SurveyMapper<Serializable> surveyMapper;
    ...
    public Survey getSurveyBySurveyDefinitionId(SurveyDefinition surveyDefinition,
            TbSurveyQestnrPerson tbSurveyQestnrPerson) {

        LoginUser loginUser = SessionContextUtil.getLoginUser();

        TbSurveyQestnr tbSurveyQestnr = tbSurveyQestnrMapper.selectBySurveyDefinitionId(surveyDefinition.getId());
        ...
        if (tbSurveyQestnrPerson == null || tbSurveyQestnrPerson.getSurveyId() == null) {
            Set<Survey> userSurveyEntries = this.survey_findUserEntriesByTypeIdAndLogin(surveyDefinition.getId(),
                    loginUser.getPersonId());

            if (userSurveyEntries == null || userSurveyEntries.size() == 0) {
                survey = new Survey();
                survey.setVersion(0);
                survey.setTypeId(surveyDefinition.getId());
                survey.setTypeName(surveyDefinition.getName());
                survey.setCreationDate(new Date());
                survey.setLastUpdateDate(new Date());
                survey.setLogin(loginUser.getPersonId());
                survey.setFirstName(loginUser.getTbComPerson().getKoreanNm());
                survey.setMiddleName(null);
                survey.setLastName(null);
                survey.setEmail(loginUser.getTbComPerson().getEmailAdres());
                survey.setIpAddress(RequestContextUtil.getClientIpAddress());
                survey.setStatus(SurveyStatus.I);

                survey = surveyDAO.merge(survey);

                surveyDAO.initialize(survey, surveyDefinition);
            } else {
                survey = userSurveyEntries.iterator().next();
            }
        } else {
            survey = surveyDAO.findById(tbSurveyQestnrPerson.getSurveyId());
        }
                ...
                for (QuestionAnswer questionAnswer : surveyPage.getQuestionAnswers()) {
                    ...
                    case YES_NO_DROPDOWN:
                        questionText = questionText.replace(
                                "${p" + surveyPage.getOrder() + "q" + questionAnswer.getQuestion().getOrder() + "}",
                                (questionAnswer.getStringAnswerValue() == null ? ""
                        ...
                        for (QuestionOption option : questionAnswer.getQuestion().getOptions()) {
                            if (option.getValue().equals(questionAnswer.getStringAnswerValue())) {
                                questionText = questionText
                                        .replace(
                                                "${p" + surveyPage.getOrder() + "q"
                                                        + questionAnswer.getQuestion().getOrder() + "}",
    ...
    public SurveyPage surveyPage_get(Long surveyId, Short pageOrder, final String dateFormat) {
        ...
        for (QuestionAnswer questionAnswer : surveyPage.getQuestionAnswers()) {
            if (questionAnswer.getQuestion().getType() == QuestionType.FILE_UPLOAD) {
                SurveyDocument surveyDocument = surveyDocumentDAO.findBySurveyIdAndQuestionId(surveyId,
                        questionAnswer.getQuestion().getId());
                if (surveyDocument != null) {
                    questionAnswer.setSurveyDocument(surveyDocumentDAO.findBySurveyIdAndQuestionId(surveyId,
                            questionAnswer.getQuestion().getId()));
                }
            }
        }
    ...
    public void surveyPage_update(SurveyPage surveyPage, String dateFormat) {
        for (QuestionAnswer questionAnswer : surveyPage.getQuestionAnswers()) {
            if (questionAnswer.getQuestion().getType() == QuestionType.FILE_UPLOAD) {
                if (questionAnswer.getSurveyDocument() != null
                        && questionAnswer.getSurveyDocument().getContent().length > 0) {
                    // delete any question documents
                    surveyDocumentDAO.deleteBySurveyIdAndQuestionId(surveyPage.getSurvey().getId(),
                            questionAnswer.getQuestion().getId());
        ...
        // checks if there is any branching logic on this page and update the visibility
        // of following pages if necessary.
        SurveyDefinitionPage surveyDefinitionPage = surveyDefinitionPageDAO
                .findByOrder(surveyPage.getSurvey().getTypeId(), surveyPage.getOrder());
        ...
        if (surveyDefinitionPage.hasLogic()) {
            ...
            if (surveyDefinitionPage.getPageLogic().getJumpToPageOrder() == 0) {
                for (Short i = (short) (surveyPage.getOrder() + 1); i <= surveyDefinitionPage.getSurveyDefinition()
                        .getPages().size(); i++) {
                    log.info("!!!!!!!!!!!!! updating to " + visibility + " page order" + i);
                    surveyDAO.updatePageVisibility(surveyDefinitionPage.getSurveyDefinition().getId(),
                            surveyPage.getSurvey().getId(), i, visibility);
                }
            } else {
                for (Short i = (short) (surveyPage.getOrder() + 1); i < surveyDefinitionPage.getPageLogic()
                        .getJumpToPageOrder(); i++) {
                    log.info("!!!!!!!!!!!!! updating to " + visibility + " page order" + i);
                    surveyDAO.updatePageVisibility(surveyDefinitionPage.getSurveyDefinition().getId(),
                            surveyPage.getSurvey().getId(), i, visibility);
                }
    ...
    public SurveyPage surveyPage_updateSettings(SurveyPage surveyPage) {
        ...
        for (QuestionAnswer questionAnswer : surveyPage.getQuestionAnswers()) {
            questionAnswer.setQuestion(questionDAO.findByOrder(surveyPage.getSurvey().getTypeId(),
                    surveyPage.getOrder(), questionAnswer.getOrder()));

            // If upload file question mark if file already loaded
            if (questionAnswer.getQuestion().getType() == QuestionType.FILE_UPLOAD) {
                SurveyDocument surveyDocument = surveyDocumentDAO.findBySurveyIdAndQuestionId(
                        surveyPage.getSurvey().getId(), questionAnswer.getQuestion().getId());
                if (surveyDocument != null) {
    ...
    public Set<Survey> surveyDefinition_findAll() throws DataAccessException {
     ...
     */
    @Transactional(readOnly = false)
    public int modSurveyStatus(long surveyId, String login) {
        return surveyMapper.modSurveyStatus(surveyId, login);
    }
