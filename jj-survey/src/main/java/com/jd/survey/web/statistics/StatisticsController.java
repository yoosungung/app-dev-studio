...
public class StatisticsController {
    ...
    void populateModel(Model uiModel, Long surveyDefinitionId, Question question, User user) {
        try {
            ...
            uiModel.addAttribute("questionStatistics",
                    surveyService.questionStatistic_getStatistics(question, recordCount));
        } catch (Exception e) {
     ...
     */
    @Secured({ "ROLE_ADMIN", "ROLE_SURVEY_ADMIN" })
    @RequestMapping(value = "/list2", produces = "text/html", method = RequestMethod.GET)
    public String listSurveyEntries2(@RequestParam(value = "sid", required = true) Long surveyDefinitionId,
            @RequestParam(value = "qid", required = false) Long questionId, Model uiModel, Principal principal,
            HttpServletRequest httpServletRequest) {
        ...
        try {
            ...
            for (SurveyDefinitionPage page : surveyDefinition.getPages()) {
                for (Question question : page.getQuestions()) {
                    List<QuestionStatistic> questionStatistics = surveyService.questionStatistic_getStatistics(question,
                            recordCount);
    ...
    private PieDataset createDataset(Long questionId, Long recordCount) {
        try {
            ...
            for (QuestionStatistic questionStatistic : questionStatistics) {
                // double percentage =
                // ((double)questionStatistic.getCount()/(double)recordCount) * 100;
    ...
    private JFreeChart createChart(PieDataset pieDataset, String title) {
        try {
            ...
            // Color[] colors = {new Color(170, 195, 217, 255),new Color(246, 140, 31,
            // 255),new Color(204, 204, 204, 255),new Color(231, 238, 144, 255),new
            // Color(51, 51, 51, 255),new Color(101, 125, 151, 255),new Color(0, 102, 255,
            // 255)};
