...
public class ReportDAOImpl implements ReportDAO {
    ...
    public List<Map<String, Object>> getSurveyData(Long surveyDefinitionId) {
        String selectSql = "select s.creation_date, s.last_update_date, s.first_name, s.last_name, s.middle_name, s.ip_address, s.login, s.status, s.submission_date, s.type_name, d.* from survey_data_"
                + surveyDefinitionId + " d inner join survey s on (s.id=d.survey_id)";
