...
public class UserDAOImpl extends AbstractJpaDao<User> implements UserDAO {
    ...
    @SuppressWarnings("unchecked")
    @Transactional
    public Set<User> findAllExternal(int startResult, int maxRows, String keywords) throws DataAccessException {
        Query query = createNamedQuery("User.findAllExternalAndSearch", startResult, maxRows);
    ...
    public User findById(Long id) throws DataAccessException {
        try {
            Query query = createNamedQuery("User.findById", -1, -1);
    ...
    @Transactional
    public Long getCountExternal(String keywords) throws DataAccessException {
        try {
            Query query = createNamedQuery("User.getCountExternalAndSearch", -1, -1);
            query.setParameter("keywords", "%" + keywords + "%");
            return (Long) query.getSingleResult();
        } catch (NoResultException nre) {
    ...
    public User findByLogin(String login) throws DataAccessException {
        try {
            Query query = createNamedQuery("User.findByLogin", -1, -1);
    ...
    public User findByEmail(String email) throws DataAccessException {
        try {
            Query query = createNamedQuery("User.findByEmail", -1, -1);
    ...
    public Set<User> searchByFirstName(String firstName) throws DataAccessException {
        Query query = createNamedQuery("User.searchByFirstName", -1, -1);
    ...
    public Set<User> searchByLastName(String lastName) throws DataAccessException {
        Query query = createNamedQuery("User.searchByLastName", -1, -1);
    ...
    public Set<User> searchByFirstNameAndLastName(String firstName, String lastName) throws DataAccessException {
        Query query = createNamedQuery("User.searchByFirstNameAndLastName", -1, -1);
        query.setParameter("firstName", "%" + firstName + "%");
        query.setParameter("lastName", "%" + lastName + "%");
        return new LinkedHashSet<User>(query.getResultList());
    }
    ...
    public Set<User> searchByLogin(String login) throws DataAccessException {
        Query query = createNamedQuery("User.searchByLogin", -1, -1);
        query.setParameter("login", "%" + login + "%");
        return new LinkedHashSet<User>(query.getResultList());
    }
    ...
    public Set<User> searchByEmail(String email) throws DataAccessException {
        Query query = createNamedQuery("User.searchByEmail", -1, -1);
        query.setParameter("email", "%" + email + "%");
        return new LinkedHashSet<User>(query.getResultList());
    }
