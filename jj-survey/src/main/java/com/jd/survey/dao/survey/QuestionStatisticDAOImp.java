...
public class QuestionStatisticDAOImp implements QuestionStatisticDAO {
     ...
     * @return
     */
    public List<QuestionStatistic> getValues(Question question) {
        ...
        stringBuilder.append("select d." + columnName + " as col from survey_data_" + surveyDefinitionId
    ...
    /**
     * Returns frequency statistics for a choice matrix question' answers (Value,
     * Count)
     * 
     * @param question
     * @return
    ...
    private List<QuestionStatistic> getArrayFrequencyStatistics(Question question, final Long totalRecordCount) {
        ...
        for (QuestionOption questionOption : question.getOptions()) {
            ...
            stringBuilder.append("select d." + columnName + " as col, count(*) as total ");
            stringBuilder.append(" from survey_data_" + surveyDefinitionId
                    + " d inner join survey s on (s.id=d.survey_id and s.status='S') where " + columnName + "=1");
            stringBuilder.append(" group by d." + columnName);
            String selectSQLStatement = stringBuilder.toString();
            List<QuestionStatistic> questionCellStatistics = this.jdbcTemplate.query(selectSQLStatement,
                    new RowMapper<QuestionStatistic>() {
    ...
    /**
     * Returns descriptive statistics for numeric question' answers (minimum,
    ...
    private List<QuestionStatistic> getDescriptiveStatistics(Question question, final Long totalRecordCount) {
        ...
        final String columnName = "p" + pageOrder + "q" + questionOrder;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("select MIN(d." + columnName + ") as min ,MAX(d." + columnName + ") as max ,AVG(d."
                + columnName + ") as avg ,STDDEV_SAMP(d." + columnName + ") as std ");
        stringBuilder.append(" from survey_data_" + surveyDefinitionId
                + " d inner join survey s on (s.id=d.survey_id and s.status='S')");
        String selectSQLStatement = stringBuilder.toString();
    ...
    /**
     * Returns descriptive statistics for numeric matrix question' answers (minimum,
     * maximum, average, standard deviation)
     * 
     * @param question
     * @return
