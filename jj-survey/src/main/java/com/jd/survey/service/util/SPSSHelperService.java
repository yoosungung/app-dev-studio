...
public class SPSSHelperService {
     ...
     * MATRIX Questions
     */
    private StringBuffer getSPSSVariableName(Question question, QuestionRowLabel questionRowLabel,
            QuestionColumnLabel questionColumnLabel) {
        return new StringBuffer().append("P" + question.getPage().getOrder() + "Q" + question.getOrder() + "R"
    ...
    private StringBuffer getSPSSVariableName(QuestionOption option) {
        return new StringBuffer().append("P" + option.getQuestion().getPage().getOrder() + "Q"
                + option.getQuestion().getOrder() + "_" + option.getOrder());
    }
    ...
    private String getSPSSVariableLabel(Question question) {
        String name = new StringBuffer().append("P" + question.getPage().getOrder().toString().replace("\"", "\"\"")
                + "-" + question.getQuestionText().replace("\"", "\"\"")).toString();
        if (name.length() > 60) {
    ...
    private String getSPSSVariableLabel(QuestionOption option) {
        String name = new StringBuffer()
                .append("P" + option.getQuestion().getPage().getOrder().toString().replace("\"", "\"\"") + "-"
                        + option.getQuestion().getQuestionText().replace("\"", "\"\"") + "["
                        + option.getText().replace("\"", "\"\"") + "]")
                .toString();
        if (name.length() > 60) {
            return "\"" + name.substring(0, 60) + "\"";
        } else {
    ...
    private String getSPSSVariableLabel(Question question, QuestionRowLabel questionRowLabel,
            QuestionColumnLabel questionColumnLabel) {
        String name = new StringBuffer()
                .append("P" + questionRowLabel.getQuestion().getPage().getOrder().toString().replace("\"", "\"\"") + "-"
                    ...
                    case YES_NO_DROPDOWN_MATRIX:
                        for (QuestionRowLabel questionRowLabel : question.getRowLabels()) {
                            for (QuestionColumnLabel questionColumnLabel : question.getColumnLabels()) {
                                sb.append("VALUE LABELS ")
                                        .append(getSPSSVariableName(question, questionRowLabel, questionColumnLabel))
                                        .append(" 1 'Yes' 0 'No'.\n");
                            }
                        }
                        break;
                ...
                for (Question question : page.getQuestions()) {
                    ...
                    if (question.getType().getIsMatrix()) {
                        ...
                        for (QuestionRowLabel questionRowLabel : question.getRowLabels()) {
                            // for each column
                            for (QuestionColumnLabel questionColumnLabel : question.getColumnLabels()) {
                                sb.append("VARIABLE LABELS ")
                                        .append(getSPSSVariableName(question, questionRowLabel, questionColumnLabel))
                                        .append(" ")
                                        .append(getSPSSVariableLabel(question, questionRowLabel, questionColumnLabel))
                                        .append(".\n");
                            }
                        }
                    } else {
                        ...
                        if (question.getType().getIsMultipleValue()) {
                            for (QuestionOption option : question.getOptions()) {
                                sb.append("VARIABLE LABELS ").append(getSPSSVariableName(option)).append(" ")
                                        .append(getSPSSVariableLabel(option)).append(".\n");
                            }
                        }
                        // Single value Questions
                        else {
                            sb.append("VARIABLE LABELS ").append(getSPSSVariableName(question)).append(" ")
                                    .append(getSPSSVariableLabel(question)).append(".\n");
                        }
                    }
                }
            }
    ...
    public byte[] getSurveyDefinitionSPSSMetadata(Long surveyDefinitionId, String dataFileName) {
        try {
            ...
            metadataStringBuffer.append("GET DATA  " + "/TYPE=TXT\n" + "/FILE='" + dataFileName + "'\n"
    ...
    public byte[] getSurveyDefinitionSPSSData(Long surveyDefinitionId) {
        try {
            ...
            for (Map<String, Object> record : surveys) {
                // dataStringBuffer.append(record.get("survey_id") == null ? "" : "\"" +
                // record.get("survey_id").toString().replace("\"", "\"\"") +"\"," );
                // dataStringBuffer.append(record.get("type_name") == null ? "" : "\"" +
                // record.get("type_name").toString().replace("\"", "\"\"") +"\"," );
                // dataStringBuffer.append(record.get("login") == null ? "" : "\"" +
                // record.get("login").toString().replace("\"", "\"\"") +"\"," );
                // dataStringBuffer.append(record.get("submission_date") == null ? "" : "\"" +
                // record.get("creation_date").toString().replace("\"", "\"\"") +"\"," );
                // dataStringBuffer.append(record.get("creation_date") == null ? "" : "\"" +
                // record.get("last_update_date").toString().replace("\"", "\"\"") +"\"," );
                // dataStringBuffer.append(record.get("last_update_date") == null ? "" : "\"" +
                // record.get("last_update_date").toString().replace("\"", "\"\"") +"\"," );
